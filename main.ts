...
function calculateBatteryVoltage () {
    batteryMilliVolt = Math.round(pins.analogReadPin(AnalogReadWritePin.P0) * batteryFactor * 0.05 + batteryMilliVolt * 0.95)
}
...
basic.forever(function () {
    ...
    led.toggle(4, 0)
    // basic.clearScreen()
    if (pins.analogReadPin(AnalogReadWritePin.P0) < 600 && pins.analogReadPin(AnalogReadWritePin.P0) >= 400) {
        iconBatteryCharging()
    } else if (batteryEmpty || batteryMilliVolt < lowBatteryLimit && pins.analogReadPin(AnalogReadWritePin.P0) > 300) {
        lowBattery()
    } else {
        mainScreen()
        buzzer = 0
